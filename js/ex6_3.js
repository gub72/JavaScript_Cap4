...
frm.btListar.addEventListener("click", () => {//reduce
    if (carros.length == 0) {
...
frm.btFiltrar.addEventListener("click", () => {//filter
    ...
    if (carrosFilter.length == 0) {
        alert("Nao a carros com preco inferior ou igual ao solicitado.")
        return
    }
    let lista = ""
    ...
    resp.innerText = `Carros ate R$: ${maximo.toFixed(2)}\n${"-".repeat(40)}\n${lista}`
})

frm.btSimular.addEventListener("click", () => {//map
